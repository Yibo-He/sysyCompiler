...
/**************************************************/
//构造函数
FunCallAST::FunCallAST(Token1* token1){
    type = FunCall;
    //处理函数名
    BaseEntry* tmp = findString(token1->strPtr);
    if(!tmp){
        if(Debug) printf("Error in function 'ast.cpp->FunCallAST::FunCallAST(Token1* token1)', function not declared!\n");
        if(Debug) printf("function name: %s\n",token1->strPtr->c_str());
    }
    if(tmp->type != funSym){
        if(Debug) printf("Error in function 'ast.cpp->FunCallAST::FunCallAST(Token1* token1)', not function name!\n");
...
//BinaryExpAST:二元表达式
//读入INT-CONST型token
BinaryExpAST::BinaryExpAST(Token1* token1): \
val(token1->val),isConst_(true),isCaled_(false), \
firstExp(nullptr),secExp(nullptr){
    type = BinaryExp;
}
//常规二元表达式，两个值与一个运算符
BinaryExpAST::BinaryExpAST(BaseAST* a, BaseAST*b, Token1* token1): \
val(0),isConst_(false),isCaled_(false), \
firstExp(a),secExp(b),op(token1->type){
    type = BinaryExp;
}
//析构函数
...
//UnaryExpAST:一元表达式
//构造函数
UnaryExpAST::UnaryExpAST(BaseAST* a, Token1* token1): \
val(0),op(token1->type),firstExp(a), \
...
*/
//token1->type不可能是Minus_Not,这个类型是引入的辅助值
void UnaryExpAST::dealOp(Token1* token1){
    switch (op){
        ...
        case NOT://!
            ...
            break;
        case Minus_Not://"-!"
            switch (token1->type){ //Minus_Not: +、-:Minus_Not, !:Minus_Not_Not
                case PLUS:
                    op = Minus_Not;
                    break;
                case MINUS:
            ...
            break;   
        case Minus_Not_Not://"-!!"
            switch (token1->type){ //Minus_Not_Not: +、-:Minus_Not_Not, !:Minus_Not
                case PLUS:
                    op = Minus_Not_Not;
                    break;
                case MINUS:
...
//Lval:变量引用
LvalAST::LvalAST(Token1* token1, BaseAST* ast): \
isConst_(false),isCaled_(false),isLeft_(false), \
val(0){
    type=Lval;
    BaseEntry* tmp = findString(token1->strPtr);
    if(!tmp){
        if(Debug) printf("Error in function 'ast.cpp->LvalAST::LvalAST(Token1* token1, BaseAST* ast)', variable not declared!\n");
        if(Debug) printf("variable name: %s\n",token1->strPtr->c_str());
    }
    if(tmp->type != varSym){
        if(Debug) printf("Error in function 'ast.cpp->LvalAST::LvalAST(Token1* token1, BaseAST* ast)', not variable name!\n");
        if(Debug) printf("name: %s\n",token1->strPtr->c_str());
    }
    sym = tmp;
...
map<int, BaseAST* > ListTree::Flatten(const vector<int> &dims){
	map<int, BaseAST* > ret; 
	vector<int> savedim = dims; 
	_Flatten_(savedim, ret, 0, savedim.back());
    //************************
    for(int i=0; i<dims.back(); ++i){
        auto it = ret.find(i);
        if(it==ret.end()){
            BaseAST* ptr = new BinaryExpAST(new Token1(INT_CONST, 0));
            ret.insert(pair<int, BaseAST*>(i,ptr));
        }
    }
	return ret;
}
