...
class Token1{
    public:
      ...
      Token1(tokenType type_):type(type_){};
      Token1(tokenType type_, int val_):type(type_),val(val_){};
      Token1(tokenType type_, const char* c):type(type_),strPtr(insertStrtab(c)){};
      Token1(tokenType type_, const char* c, int lineno):type(type_),strPtr(insertStrtab(c)),lineNum(lineno){};

      /*void printToken(){
         printf("Get a Token1: %s,  ",token2string[type-260].c_str());
